...
class ProxyManager:
    ...
    def _fetch_iproyal_proxies(
        self, api_key: str, country: str, length: int,
        ...
        Args:
            ...
            session_id: Уникальный ID сессии (для fresh-прокси)
        Returns:
        ...
        # Если передан session_id — используем его как имя сессии,
        # иначе используем сам api_key (поведение для «общей» сессии).
    ...
    # 🔥 НОВОЕ: свежий прокси на каждую регистрацию
    def get_fresh_proxy(self) -> Optional[Dict]:
        """
        Получить свежий прокси.
        Для IPRoyal — запрашивает новую сессию с уникальным session_id,
        чтобы каждый аккаунт шёл с нового IP.
        Для других режимов — обычный round-robin.
        """
        from .logger import log
        if self.config.get("proxy.mode") == "iproyal":
            api_key = self.config.get("proxy.iproyal_api_key", "")
            ...
            country = self.config.get("proxy.iproyal_country", "all")
            length = self.config.get("proxy.iproyal_length", 30)
            # Уникальный session_id для каждого аккаунта
            fresh_session = f"{api_key}:s_{uuid.uuid4().hex[:10]}"
            proxies = self._fetch_iproyal_proxies(
                api_key, country, length, session_id=fresh_session
            )
            if proxies:
                log.debug(f"🔄 Fresh IPRoyal session: {fresh_session}")
                return proxies[0]
            log.warning("Не удалось получить fresh-прокси, fallback на get_next()")
            return self.get_next()
        return self.get_next()
    ...
    def get_playwright_proxy(self) -> Optional[Dict]:
        ...
        if not proxy:
            return None
        result = {"server": f"{proxy['type']}://{proxy['server']}"}
